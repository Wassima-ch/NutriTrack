...
import { 
  ...
  Scale, Target as TargetIcon, Ruler, CalendarDays, User, Activity, Edit3, Check, Camera, Plus, X
} from 'lucide-react-native';

// --- ÉTAPE 1 : SORTIR LE COMPOSANT ProfileItem ICI POUR FIXER LE CLAVIER ---
...
export default function ProfileScreen({ navigation }: any) {
  ...
  const handleSave = async () => {
    ...
    const taille = parseInt(data.taille);
    const poids = parseFloat(data.poids);

    if (age < 1 || age > 110) return Alert.alert("Données invalides","Veuillez entrer un âge réaliste.");
    if (taille < 40 || taille > 250) return Alert.alert("Données invalides","Veuillez entrer une taille réaliste.");
    if (poids < 2 || poids > 400) return Alert.alert("Données invalides", "Veuillez entrer un poids réaliste.");
  ...
  const handleAddPreference = () => {
    if (newPref.trim().length > 0) {
      ...
      if (!currentPrefs.includes(newPref.trim())) {
        setData({ ...data, preferences: [...currentPrefs, newPref.trim()] });
      }
      setNewPref('');
  ...
  if (loading) return (
    <View className="flex-1 bg-fresh justify-center items-center"><ActivityIndicator size="large" color="#7FB058" /></View>
  );
    ...
    <SafeAreaView className="flex-1 bg-fresh">
      ...
      <View className="flex-row items-center justify-between px-6 py-2">
        ...
        <TouchableOpacity 
          ...
          className={`p-2 rounded-xl ${isEditing ? 'bg-primary' : 'bg-white border border-secondary/20'}`}
        >
      ...
      <ScrollView className="flex-1 px-6" showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        <View className="items-center my-6">
          <TouchableOpacity onPress={pickImage} className="relative shadow-lg">
            <View className="bg-white p-1 rounded-full border-4 border-primary/10">
              {data?.profileImage ? (
                <Image source={{ uri: data.profileImage }} className="w-24 h-24 rounded-full" />
              ) : (
                <View className="w-24 h-24 rounded-full bg-fresh items-center justify-center">
                   <UserCircle2 size={70} color="#7FB058" strokeWidth={1} />
                </View>
              )}
            </View>
          ...
          {isEditing ? (
            <View className="flex-row mt-4 space-x-2">
              <TextInput className="text-xl font-black text-mainText border-b-2 border-primary/30 min-w-[80px] text-center" value={data?.prenom} onChangeText={(t) => setData({...data, prenom: t})} placeholder="Prénom" />
              <TextInput className="text-xl font-black text-mainText border-b-2 border-primary/30 min-w-[80px] text-center" value={data?.nom} onChangeText={(t) => setData({...data, nom: t})} placeholder="Nom" />
            </View>
          ) : (
        ...
        <View className="flex-row justify-between">
            <View style={{width: '48%'}}>
              <ProfileItem icon={CalendarDays} label="Âge" value={data?.age} field="age" keyboardType="numeric" isEditing={isEditing} data={data} setData={setData} />
            </View>
            <View style={{width: '48%'}}>
              <ProfileItem icon={User} label="Genre" value={data?.sexe || data?.genre} field="sexe" isEditing={isEditing} data={data} setData={setData} />
            </View>
        </View>
        ...
        <View className="flex-row items-center justify-between mt-6 mb-3">
          <Text className="text-mainText font-black text-lg italic">Mes Préférences</Text>
          {isEditing && (
            <TouchableOpacity onPress={() => setModalVisible(true)} className="flex-row items-center bg-primary/20 px-3 py-1.5 rounded-full">
              <Plus size={14} color="#7FB058" />
              <Text className="text-primary font-bold ml-1 text-xs">Ajouter</Text>
            </TouchableOpacity>
          )}
        </View>
        ...
        <View className="flex-row flex-wrap gap-2 mb-6">
          {data?.preferences?.map((pref: string, index: number) => (
            <View key={index} className="flex-row items-center bg-primary px-4 py-2 rounded-full shadow-sm">
              <Text className="text-white font-bold text-xs">{pref}</Text>
              {isEditing && (
                <TouchableOpacity onPress={() => setData({...data, preferences: data.preferences.filter((p:any)=>p!==pref)})} className="ml-2">
        ...
        <View className="bg-white p-4 rounded-2xl mb-3 flex-row items-center justify-between border border-secondary/20 shadow-sm">
        ...
        <View className="bg-white p-4 rounded-2xl mb-8 flex-row items-center border border-secondary/20 shadow-sm">
            ...
            <View>
              <Text className="text-mutedText text-[10px] font-bold uppercase tracking-widest">Version</Text>
              <Text className="text-mainText font-bold">1.0.8 - Stable</Text>
            </View>
        </View>
        ...
        <TouchableOpacity onPress={() => signOut(auth).then(()=>navigation.replace('Login'))} className="flex-row items-center justify-center bg-red-50 p-5 rounded-[25px] mb-12 border border-red-100">
          <LogOut size={20} color="#FF8080" />
          <Text className="text-[#FF8080] font-bold text-lg ml-2 uppercase tracking-tighter">Déconnexion</Text>
        </TouchableOpacity>
      </ScrollView>
      ...
      <Modal visible={modalVisible} transparent={true} animationType="fade" onRequestClose={() => setModalVisible(false)}>
        <View className="flex-1 justify-center items-center bg-black/60 px-6">
          <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} className="w-full">
            <View className="bg-white p-6 rounded-[30px] shadow-2xl border-2 border-secondary">
              <Text className="text-xl font-black text-mainText mb-4">Ajouter une préférence</Text>
              <TextInput 
                placeholder="Ex: Sans Lactose..."
                className="bg-fresh p-4 rounded-2xl mb-6 border border-secondary/20 font-bold"
                value={newPref}
                onChangeText={setNewPref}
                autoFocus={true}
              />
              <View className="flex-row space-x-3">
                <TouchableOpacity onPress={() => setModalVisible(false)} className="flex-1 p-4 rounded-2xl bg-gray-100 border border-transparent">
                  <Text className="text-center font-bold text-mutedText">Annuler</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={handleAddPreference} className="flex-1 p-4 rounded-2xl bg-primary border-2 border-secondary/30">
