...
export default function SignupPrefsScreen({ route, navigation }: any) {
  ...
  const handleFinish = async () => {
    try {
      ...
      Alert.alert("Erreur", "Impossible de finaliser votre profil. Vérifiez votre connexion.");
    }
  };
  return (
      ...
      <ScrollView className="flex-1 mt-10" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 20 }}>
          ...
          {PRESET_OPTIONS.map((item) => {
            ...
            return (
              ...
              >
                ...
                <Text numberOfLines={1} className="font-bold text-[14px] text-mainText">
                  {item.label}
                </Text>
                {isSelected && (
        ...
        <View className="mt-4 px-1">
          ...
          <View className="flex-row flex-wrap">
            {prefs.filter(p => !PRESET_OPTIONS.some(o => o.id === p)).map((p) => (
              <View 
                key={p} 
