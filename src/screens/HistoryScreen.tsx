...
export default function HistoryScreen({ navigation }: any) {
  ...
  const [selectedMeal, setSelectedMeal] = useState<any>(null); // Pour la Pop-up
    ...
    Alert.alert(
      ...
      [{ text: "Annuler", style: "cancel" },
       { text: "Supprimer", style: "destructive", onPress: async () => {
            try { await deleteDoc(doc(db, "meals", mealId)); setSelectedMeal(null); } 
  ...
  return (
    <View className="flex-1 bg-fresh">
      <SafeAreaView className="flex-1">
        ...
        {loading ? (
          <View className="flex-1 justify-center"><ActivityIndicator size="large" color="#A3C981" /></View>
        ) : (
          <SectionList
            sections={sections}
              ...
              >
                ...
                <View className="flex-1 justify-center">
                  ...
                  <View className="flex-row">
                    <View className="bg-primary/10 px-2 py-0.5 rounded-xl">
                      <Text className="text-primary font-black text-sm">{item.calories} <Text className="text-[9px]">KCAL</Text></Text>
                    </View>
                  </View>
                </View>
                ...
                <View className="items-end justify-between min-h-[50px]">
                  <View className="bg-gray-50 px-3 py-1.5 rounded-xl border border-gray-100">
                    <Text className="text-[9px] text-mutedText font-black uppercase">P:{item.proteins} G:{item.carbs} L:{item.fats}</Text>
                  </View>
            ...
            ListEmptyComponent={() => (
              <View className="items-center mt-32">
                <View className="bg-gray-50 p-6 rounded-full"><History size={60} color="#D1D5DB" /></View>
            ...
            <Pressable className="bg-white rounded-t-[40px] p-8 shadow-2xl">
              ...
              <View className="flex-row justify-between mb-8">
                <View className="items-center flex-1 border-r border-gray-100">
                  ...
                  <Text className="font-black text-lg mt-1">{selectedMeal?.calories}</Text>
                ...
                <View className="items-center flex-1">
                  <Utensils size={20} color="#A3C981" />
                  <Text className="font-black text-lg mt-1">{selectedMeal?.proteins + selectedMeal?.carbs + selectedMeal?.fats}g</Text>
                ...
                <View className="flex-row justify-around">
                  <View className="items-center">
                    <View className="w-12 h-12 bg-white rounded-2xl items-center justify-center border border-primary/10 mb-2">
                      <Text className="text-primary font-black">{selectedMeal?.proteins}g</Text>
                    </View>
                  ...
                  <View className="items-center">
                    <View className="w-12 h-12 bg-white rounded-2xl items-center justify-center border border-yellow-100 mb-2">
                      <Text className="text-yellow-500 font-black">{selectedMeal?.carbs}g</Text>
                    </View>
                  ...
                  <View className="items-center">
                    <View className="w-12 h-12 bg-white rounded-2xl items-center justify-center border border-red-100 mb-2">
                      <Text className="text-red-400 font-black">{selectedMeal?.fats}g</Text>
                    </View>
